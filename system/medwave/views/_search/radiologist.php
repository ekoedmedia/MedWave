<?php
	...
	// Get Records from Specified Time Range
		...
		if ($_GET['order'] == 'desc')
			$sql = "SELECT * FROM radiology_record WHERE radiologist_name=:user AND test_date BETWEEN :from AND :to ORDER BY test_date DESC";
		else 
			$sql = "SELECT * FROM radiology_record WHERE radiologist_name=:user AND test_date BETWEEN :from AND :to ORDER BY test_date ASC";
		$stmt = $dbcon->prepare($sql);
